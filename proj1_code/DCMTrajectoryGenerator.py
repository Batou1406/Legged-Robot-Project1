...
class DCMTrajectoryGenerator:
    ...
    def getCoMTrajectory(self,com_ini):
        ...
        for kk in range(0,self.CoM.shape[0]-1):
            ...
            self.CoM[kk+1]= self.Com[KK] + self.timeStep*self.CoMDot[kk+1]#Simple euler numerical integration -> retrograde
    ...
    def findBoundryConditionsOfDCMDoubleSupport(self):
        ...
        for stepNumber in range(np.size(self.CoP,0)):
            if stepNumber == 0: #Boundary conditions of double support for the first step(equation 11b and 12b in Jupyter notebook)
                ...
                self.finalDCMForDS[stepNumber] =  self.COP[0] + (self.initialDCMForDS[0]-self.COP[0])*np.exp(self.omega*(1-self.alpha)*self.dsTime)# use (12b)
                ...
                self.initialDCMForDS[stepNumber] = self.COP[stepNumber-1] + (self.DCMForEndOfStep[stepNumber-1] - self.COP[stepNumber-1])*exp(-self.omega*self.alpa*self.dsTime) #use equation(11)
                self.finalDCMForDS[stepNumber] = self.COP[stepNumber] + (self.DCMForEndOfStep[stepNumber-1] - self.COP[stepNumber])*exp(self.omega*(1-self.alpa)*self.dsTime) #use equation(12)
